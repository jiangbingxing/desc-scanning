...
public class gbase extends Analysiser {
	private static Logger logger = Logger.getLogger(gbase.class);
			...
			for (int i = 0; i < hangs.length; i++) {
				...
				String path=null;
				 if (lie[7].equals("grep")) {
					continue;
				}else if(PatternUtil.getMatch(hangs[i],"/gbase").size()>0){
					String temp=hangs[i].split("/gbase")[1];
					path=temp.split(" ")[0];
					logger.error(path);
				...
				if(portString!=null)
				{
				  ...
				  jsonObject.addProperty("id",BDid.Gbase);
				   jsonObject.addProperty(elements.attr("property1"),username);
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(1).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(1).attr("dataType1"));
				   jsonObject.addProperty("astType",elements.attr("astType"));
				   jsonObjects.add(jsonObject);
			}
		}
		totalString=FindExec.commandExec(elements2.get(1).attr("command2"));
		if (totalString==null) {
			logger.error("the password is erro or the network no connection");
			return null;
		}
		...
		if(totalString!=null &&hangs.length>1)
		{
			for (int i = 0; i < hangs.length; i++) {
				...
				String path=null;
				 if (lie[7].equals("grep")) {
					continue;
				}else if(PatternUtil.getMatch(hangs[i],"/flume").size()>0){
					String temp=hangs[i].split("/flume")[1];
					path=temp.split(" ")[0];
					logger.error(path);
				...
				if(portString!=null)
				{
				  ...
				  jsonObject.addProperty(elements.attr("property1"),username);
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(1).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(1).attr("dataType2"));
				   jsonObject.addProperty("astType",elements.attr("astType"));
				   jsonObject.addProperty("id",BDid.Gbase);
			       jsonObjects.add(jsonObject);
				}
