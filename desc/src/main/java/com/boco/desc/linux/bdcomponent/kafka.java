...
public class kafka extends Analysiser {
	...
	public List<JsonObject> Find(Elements elements) {
		...
		String[] hangs=null;
		totalString=FindExec.commandExec(elements2.get(6).attr("command1"));
		if (totalString==null) {
			...
			for (int i = 0; i < hangs.length; i++) {
				...
				String path=null;
				 if (lie[7].equals("grep")) {
					continue;
				}else if(PatternUtil.getMatch(hangs[i],"/kafka").size()>0){
					String temp=hangs[i].split("/kafka")[1];
					path=temp.split(" ")[0];
					logger.error(path);
				...
				if(portString!=null)
				{
				   ...
				   jsonObject.addProperty("id",BDid.kafka);
				   jsonObject.addProperty(elements.attr("property1"),username);
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(6).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(6).attr("dataType1"));
				   jsonObject.addProperty("astType",elements.attr("astType"));
				 jsonObjects.add(jsonObject);
			}
		}
		totalString=FindExec.commandExec(elements2.get(6).attr("command2"));
		if (totalString==null) {
			logger.error("the password is erro or the network no connection");
			return null;
		}
			...
			for (int i = 0; i < hangs.length; i++) {
				...
				if(portString!=null)
				{
				 ...
				 jsonObject.addProperty("id",BDid.kafka);
				   ...
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(6).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(6).attr("dataType2"));
				   jsonObject.addProperty("astType",elements.attr("astType"));
