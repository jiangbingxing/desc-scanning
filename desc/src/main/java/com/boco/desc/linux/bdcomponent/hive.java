...
public class hive extends Analysiser {
			...
			for (int i = 0; i < hangs.length; i++) {
				...
				String path=null;
				 if (lie[7].equals("grep")) {
					continue;
				}else if(PatternUtil.getMatch(hangs[i],"/hive").size()>0){
					String temp=hangs[i].split("/hive")[1];
					path=temp.split(" ")[0];
					logger.error(path);
				...
				if(portString!=null)
				{
				 ...
				 jsonObject.addProperty("id",BDid.Hive);
				   jsonObject.addProperty(elements.attr("property1"),username);
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(4).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(4).attr("dataType1"));
				   jsonObject.addProperty("astType",elements.attr("astType"));
				   jsonObjects.add(jsonObject);
			}
		}
		totalString=FindExec.commandExec(elements2.get(4).attr("command2"));
		if (totalString==null) {
			logger.error("the password is erro or the network no connection");
			return null;
		}
		...
		if(totalString!=null &&hangs.length>1)
		{
			for (int i = 0; i < hangs.length; i++) {
				...
				jsonObject.addProperty("id",BDid.Hive);
				   ...
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(4).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(4).attr("dataType2"));
				   jsonObject.addProperty("astType",elements.attr("astType"));
