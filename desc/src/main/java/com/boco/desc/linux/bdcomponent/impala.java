...
public class impala extends Analysiser {
	...
	public List<JsonObject> Find(Elements elements) {
		...
		String[] hangs=null;
		totalString=FindExec.commandExec(elements2.get(5).attr("command1"));
		if (totalString==null) {
			...
			for (int i = 0; i < hangs.length; i++) {
				...
				String path=null;
				 if (lie[7].equals("grep")) {
					continue;
				}else if(PatternUtil.getMatch(hangs[i],"/impala").size()>0){
					String temp=hangs[i].split("/impala")[1];
					path=temp.split(" ")[0];
					logger.error(path);
				...
				if(portString!=null)
				{
				 ...
				 jsonObject.addProperty("id",BDid.Impala);
				   jsonObject.addProperty(elements.attr("property1"),username);
				   jsonObject.addProperty(elements.attr("property3"),path);
				   jsonObject.addProperty("typeName",elements2.get(5).attr("name"));
				   jsonObject.addProperty("dataType",elements2.get(5).attr("dataType1"));
