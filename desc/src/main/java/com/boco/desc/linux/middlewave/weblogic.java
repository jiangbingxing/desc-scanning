...
public class  weblogic extends Analysiser {
	private static Logger logger = Logger.getLogger(weblogic.class);
			...
			for (int i = 0; i < hangs.length; i++) {
				...
				if(portString!=null)
				{
				 ...
				 jsonObject.addProperty(elements.attr("property1"),username);
				   ...
				   jsonObject.addProperty("name",elements2.get(2).attr("name"));
