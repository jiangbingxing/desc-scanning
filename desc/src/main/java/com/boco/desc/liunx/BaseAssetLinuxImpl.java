...
public class BaseAssetLinuxImpl implements BaseAsset{
	...
	public AstHost getBaseAsset(Login login) {
		...
		try {
			...
			//获得资产名
			Command command = new CommandImpl(LinuxCommand.ASSET_NAME);//指令
			...
			//获得资产类型
			Command command2 = new CommandImpl(LinuxCommand.ASSET_TYPE);//指令
			...
			//获得厂商
			Command command3 = new CommandImpl(LinuxCommand.MANU_FACTURER);//指令
			...
			//获得版本号
			Command command4 = new CommandImpl(LinuxCommand.VERSION);//指令
			...
			//获得MAC地址
			Command command6= new CommandImpl(LinuxCommand.MAC_ADDRESS);//指令
			...
			//获得不同的网卡地址
			Command command7= new CommandImpl(LinuxCommand.NET_CARD_ADDRESS);
	...
	public  String getPathAndUser(Login login,String Service)
	{
		...
		try {
			UserInfo userInfo = new UserInfo(login.getIp(),login.getPort(),
					login.getUsername(), login.getPassword());
			//得到安装路径
			StringBuilder stringBuilder=new StringBuilder(LinuxCommand.COMPONENT_PATH);
			...
			//得到使用应用的用户账号
			StringBuilder stringBuilder2=new StringBuilder(LinuxCommand.COMPONENT_USER);
