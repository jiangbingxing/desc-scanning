...
public class TomcatAssetLinux {
	
	List<Component> com=new ArrayList<Component>();
	Component coms=new Component("unknow","unknow","unknow",1,"unknow", null, null, 0);
	...
	public List<Component> tomcatCompoentFind(Login login) throws IOException
	 {
		...
		com.add(coms);
		 ...
		 //从配置文件取回所有的未安装应用的 命令
		 List<String> propertiesList=LoadUtil.LoadPreperties("tomcat.properties");
				 ...
				 try {
					 ...
					 if(result3.getCode()==0)
					 {
						 ...
						 for (int i = 0; i < components.size(); i++) {
							...
							components.get(i).setName(appName);
						}
						 }
						 else {
							return com;
